...
export class Service {
  ...
  async remove (id, params) {
    const recursive = _.get(params, 'query.recursive')
    // Check id, if recursive removal we can use a null id to clean all
    if (!id && !recursive) throw new BadRequest('remove: missing \'id\'')
    debug(`method 'remove' called with 'id': ${id}`)
    // Recursive remove: delete all objects matching the prefix
    if (recursive) {
      const prefix = this.getKey(id)
      let deleted = []
      let marker
      do {
        const listParams = { Bucket: this.bucket, Prefix: prefix, Marker: marker }
        const listResponse = await this.s3Client.send(new ListObjectsCommand(listParams))
        ...
        if (keys.length > 0) {
          ...
          deleted = deleted.concat(deleteResponse.Deleted || [])
        }
        marker = listResponse.IsTruncated ? listResponse.NextMarker : undefined
      } while (marker)
      return { [this.id]: id, deleted }
    }
    // Setup command params
    const commandParams = {
      Key: this.getKey(id),
  ...
  async createMultipartUpload (data, params) {
    ...
    response = { [this.id]: data.id, ...response }
    // emit the event
  ...
  async uploadPart (data, params) {
    ...
    if (!data.UploadId) throw new BadRequest('uploadPart: missing \'data.UploadId\'')
