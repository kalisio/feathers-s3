...
export class Service {
  ...
  async createSignedUrl (command, options) {
    try {
      ...
      debug(`created signed url for command '${command.input.Command}': ${signedUrl}`)
  ...
  async get (id, params) {
    ...
    try {
      ...
      return { ok: true, status: 200, buffer, type: response.ContentType }
    } catch (error) {
  ...
  async completeMultipartUpload (data, params) {
    ...
    // send command
    try {
      await this.s3Client.send(new CompleteMultipartUploadCommand(commandParams))
      return { ok: true, status: 200 }
    } catch (error) {
      return { status: 400, statusText: error }
    }
  ...
  async uploadPart (data, params) {
    ...
    if (!data.uploadId) throw new BadRequest('proxyUpload: expected missing \'data.uploadId\' parameter')
    ...
    const commandParams = {
      ...
      Key: this.getKey(data.id),
      Bucket: this.bucket,
      Body: new Buffer(data.buffer, 'base64'),
      ContentType: data.type
    }
    // send command
    try {
      const response = await this.s3Client.send(new UploadPartCommand(commandParams))
      return { ok: true, status: 200, ETag: response.ETag }
    } catch (error) {
      return { status: 400, statusText: error }
    }
  }
